...
public class SaveState extends State {
    ...
    public void init(Window window) {
        ...
        saveStateButton1 = new Button(x1, y1, 480, 270, hoverSound, "Save State 1", saveButtonTextFont, rgba);
        saveStateButton2 = new Button(x2, y1, 480, 270, hoverSound, "Save State 2", saveButtonTextFont, rgba);
        saveStateButton3 = new Button(x3, y1, 480, 270, hoverSound, "Save State 3", saveButtonTextFont, rgba);
        saveStateButton4 = new Button(x1, y2, 480, 270, hoverSound, "Save State 4", saveButtonTextFont, rgba);
        saveStateButton5 = new Button(x2, y2, 480, 270, hoverSound, "Save State 5", saveButtonTextFont, rgba);
        saveStateButton6 = new Button(x3, y2, 480, 270, hoverSound, "Save State 6", saveButtonTextFont, rgba);

        float [] rgba2 = {0.0f, 0.0f, 0.25f, 0.65f};
        tabReturnButton = new Button(0, 0, 150, 50, hoverSound, "Return", tabReturnButtonTextFont, rgba2);

        background = new Texture("assets/ui/bg_save.jpg");
    ...
    public void enter(Window window) {
        System.out.println("Entering SaveState - Loading slot images...");
    ...
    public void update(Window window) {
        ...
        titleTextFont.drawText("Save Game State", (1920/2)-330, 20);
        GL11.glColor3f(1.0f, 1.0f, 1.0f);
    }


    // --- THE UNIVERSAL SAVE HELPER ---
    private void saveGameToSlot(int slotIndex) {
        String slotName = "save_slot_" + slotIndex;
        System.out.println("Saving game to " + slotName + "...");

        try {
            // 1. Get the memory snapshot from GameState safely!
            Data.SaveData save = ((GameState) State.game).createSaveData();

            // 2. Save the JSON data into the assets/save folder
            java.io.FileWriter writer = new java.io.FileWriter("assets/save/" + slotName + ".json");
            new com.google.gson.Gson().toJson(save, writer);
            writer.close();

            // 3. Grab the temporary screenshot we took in GameState
            java.io.File tempImage = new java.io.File("assets/save/temp_screenshot.png");
            java.io.File slotImage = new java.io.File("assets/save/" + slotName + ".png");

            // 4. COPY the temporary screenshot so it doesn't get deleted!
            if (tempImage.exists()) {
                // This copies the file and safely overwrites the old slot image if one exists!
                java.nio.file.Files.copy(tempImage.toPath(), slotImage.toPath(), java.nio.file.StandardCopyOption.REPLACE_EXISTING);

                // Instantly load the new picture onto the screen!
                slotImages[slotIndex] = new Texture("assets/save/" + slotName + ".png");
            } else {
                System.out.println("Failed to find temp screenshot!");
            }

            System.out.println("Game Saved Successfully to Slot " + slotIndex + "!");
